...
- [x] Fix word counting logic to properly strip HTML tags and count actual text words in article content (Created getWordCount helper to strip HTML tags and decode entities, created getReadingTime function using proper word count with 200 wpm, updated blog page to use new helper. All 27 tests passing.)
- [x] Fix reading time on blog listing page - ensure WordPress API returns full content not just excerpts (Added content field to posts and postsByCategory routers so blog listing page has access to full article content for accurate word counting. All 27 tests passing.)
